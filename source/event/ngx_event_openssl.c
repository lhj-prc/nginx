...
ngx_int_t ngx_ssl_certificates(ngx_conf_t* cf, ngx_ssl_t* ssl, ngx_array_t* certs,
...
static X509* ngx_ssl_load_certificate(ngx_pool_t* pool, char** err, ngx_str_t* cert,
                                      STACK_OF(X509)** chain)
{
    BIO*     bio;
    X509*    x509, *temp;
...
ngx_int_t ngx_ssl_client_certificate(ngx_conf_t* cf, ngx_ssl_t* ssl, ngx_str_t* cert,
                                     ngx_int_t depth)
{
...
ngx_int_t ngx_ssl_trusted_certificate(ngx_conf_t* cf, ngx_ssl_t* ssl, ngx_str_t* cert,
                                      ngx_int_t depth)
{
...
ngx_int_t ngx_ssl_conf_commands(ngx_conf_t* cf, ngx_ssl_t* ssl, ngx_array_t* commands)
{
    if (commands == NULL) {
        return NGX_OK;
    }
...
static ssize_t ngx_ssl_sendfile(ngx_connection_t* c, ngx_buf_t* file, size_t size)
{
#if (defined BIO_get_ktls_send && !NGX_WIN32)
...
static void ngx_ssl_connection_error(ngx_connection_t* c, int sslerr, ngx_err_t err,
                                     char* text)
{
    int         n;
    ngx_uint_t  level;
...
ngx_int_t ngx_ssl_get_curve(ngx_connection_t* c, ngx_pool_t* pool, ngx_str_t* s)
{
...
ngx_int_t ngx_ssl_get_alpn_protocol(ngx_connection_t* c, ngx_pool_t* pool, ngx_str_t* s)
{
